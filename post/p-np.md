...
뉴턴의 운동 방정식, 맥스웰의 전자기 방정식, 슈뢰딩거의 파동 방정식 등이 
자연의 복잡한 현상을 공식으로 담아내려 했던 시도이자 바로 그 증거입니다.
...
※ 2024년 바로 이 단백질의 아미노산 구조를 예측하는 인공지능 프로그램-알파폴드-로 
구글 딥마인드의 CEO 데미스 허사비스가 노벨 화학상을 수상했다는 사실 알고 계실겁니다!
...
실제로 우리가 프로그래밍을 하면서 직면하는 수많은 난제들을 나열해보면
...
수 많은 데이터가 수집되고, 정제되고, 모델로 학습되고.., 
최종적으로 그 결과는 비즈니스와 과학 전반에 적용됩니다.
하지만 그 내부를 들여다보면, 
이 역시 복잡도의 장벽에 끊임없이 부딪히고 있다는 것을 알 수 있습니다.

## 2.1 학습 문제의 본질: 최적화

딥러닝 모델을 학습시킨다는 것은 결국 손실 함수(loss function)를 최소화하는 문제입니다.
그러나 이 문제는 일반적으로 비선형, 비볼록에다, 이론적으로는 NP-hard에 가깝습니다.
...
확률적 경사하강법(Stochastic Gradient Descent, SGD)이나 
Adam 같은 최적화 알고리즘은 본질적으로 “빠르게 수렴하는 근사해”를 제공할 뿐입니다.
즉, 딥러닝은 거대한 연산 능력을 동원해도 정확한 해를 구하는 것이 아니라, 
충분히 쓸 만한 근사해를 찾는 과정이라고 볼 수 있습니다.

※ 머릿속에 떠올리시는 휴리스틱은 일반적 의미(심리학/철학/인지과학)에 가까울겁니다.
즉 빠른 결정을 내리기 위해 쓰는 간편한 추론법 정도입니다.
...
나의 직관과 확률적 간극이 얼마나 떨어져 있는지 한 번 테스트해볼까요?
...
하지만 실제 확률은 처음 고른 문이 맞을 확률 1/3, 바꿨을 때 맞을 확률 2/3이죠.
...
하지만 계산조차 NP-hard 문제 앞에서는 한계에 부딪히고 맙니다.

## 2.3 데이터베이스와 복잡도

데이터 분석에서 복잡도는 AI 학습에만 국한되지 않습니다.
예를 들어, SQL 쿼리 최적화에서 가장 중요한 문제 중 하나는 조인 순서를 결정하는 것입니다.
하지만 최적의 조인 순서를 찾는 문제는 사실상 NP-hard로 알려져 있습니다.
데이터베이스 옵티마이저는 완전탐색을 수행하는 대신, 
동적 계획법, 휴리스틱, 비용 추정 기반 탐색을 함께 사용합니다.
...
데이터 사이언티스트가 데이터를 다룬다는 것은, 단순히 수학적 계산을 넘어서
...
프론트엔드 개발은 흔히 "사용자 경험(UX)을 위한 인터페이스 작업" 정도로 생각됩니다.
...
최악의 경우 O(n) ~ O(n ⋅ m) (n=노드 수, m=스타일 규칙 수)의 비용이 발생합니다.
...
* 후손 선택자 (예: `div ul li a`): 트리 탐색으로 인해 O(n ⋅ d), d는 트리 깊이
* 조합 선택자 및 부정 조건 (예: `a[href*="news"]:not(.highlight)`): 부분집합 검증과 유사, O(2^n)까지 폭발 가능

즉, 최악의 경우 CSS 매칭은 SAT(충족 가능성 문제)와 비슷한 구조를 가집니다.
(실제 이론 연구에서도, CSS 선택자의 충돌 검사나 최적 적용 순서 문제는 NP-complete로 분류될 수 있음을 보였습니다.)
...
요소 개수 $n$이 1,000개를 넘어가는 순간 실시간성은 이미 붕괴되었다고 보면 되겠죠?
...
요청 스케줄링, DB 쿼리 최적화, 분산 트랜잭션 관리 등은 
모두 이론적으로 NP-hard 혹은 그에 준하는 복잡성을 가지고 있습니다.
...
여기서 π는 요청들의 순열, Ci(π)는 i번째 요청의 완료 시간입니다.
이 문제는 가중완료시간 스케쥴링으로 알려져 있고 NP-hard입니다.

※ 가중 완료 시간(WCT) 스케줄링은 각 작업의 가중치에 완료 시간을 
곱한 값의 합을 최소화 하는 방향으로 처리할 작업 순서를 결정하는 최적화 문제입니다!
...
가능한 조인 순서의 수는 Catalan 수로 폭발할 수 있습니다:
...
테이블 수가 12~15개를 넘어가면 완전 탐색은 포기하고, 
비용 기반 근사 탐색으로 전환하는 것이 포인트입니다.
...
하나의 좋은 사례로 DynamoDB와 MongoDB를 들여다 봅시다.
...
즉, 거대한 산업 시스템조차 CAP의 제약을 피해갈 수 없고
...
따라서 현실적 시스템은 항상 근사적 전략을 취한다고 볼 수 있습니다.
...
스케줄링의 NP-hard성, 조인 순서의 조합 폭발, 
CAP 정리의 불가능성 등이 곳곳에 산재해 있기 때문입니다.
(구글, 아마존, 오픈소스 DBMS와 같은 거대 시스템조차 
완전해 대신 근사해를 택하는 것을 확인 할 수 있습니다!)
...
$(\text{cpu}_i, \text{mem}_i, \text{io}_i)$

요구량을 갖고, 노드 j의 용량이 

$(C^{\text{cpu}}_j, C^{\text{mem}}_j, C^{\text{io}}_j)$
...
$\text{Cost}=\sum_j \alpha_j \cdot \mathbf{1}[\text{node }j\text{ used}]$ 

최소화 또는 SLO(지연/가용성) 제약 하에서의 성능 최대화입니다.
...
이번에도 실제 사례를 들어 AWS/GCP의 스케줄링이 어떻게 달라지는지 알아보겠습니다.
Kubernetes 스케줄러는 기본적으로 Score/Filter 기반 
휴리스틱(예: LeastRequested, BalancedAllocation, NodeAffinity 등)으로 탐색공간을 줄이고 있고,
GKE Autopilot 및 AWS Karpenter는 실시간 가격·가용존·인스턴스 패밀리(예: c, m, r 계열) 제약까지 얽혀서 
Online Bin Packing으로 변환되기 때문에 Preemptible/Spot을 섞을 때는 
기대 중단비용까지 포함한 확률적 목표함수를 쓰는 것이 관례로 취급됩니다.
...
단일 소스 최단경로 즉 우리가 흔히 아는 다익스트라 알고리즘은 

$O(E\log V)$

로 P에 속합니다.
그러나 멀티제약(Multi-Constraint) 라우팅—예를 들어, 

지연(latency) <= L, 대역폭 >= B, 손실률 <= p

—을 동시에 만족하는 경로를 찾는 문제는 일반적으로 NP-hard로 알려져 있습니다.
...
제약이 늘어날수록 탐색공간은 지수적으로 커지고, 
정적 최단경로는 실제 트래픽 변동(혼잡, 장애, 토폴로지 변화)로 금세 비최적화됩니다.

실제 사례로 좀 더 쉽게 이해해보겠습니다.
GCP 내부 라우팅·L4 Load Balancer는 "ECMP + 근사적 비용함수"로 빠른 재수렴을 택하고 있습니다.
또한 CDN/Anycast는 최단거리 대신 관측 기반(실측 RTT, 실패율)으로 동적 가중치를 갱신하는데, 
이것은 사실상 Online Optimization + Bandit에 가깝습니다.
결론적으로, 네트워킹도 “한 방의 최적해”가 아닌 지속적 근사가 기본 전략인 셈입니다.

---

## 5.3 Observability: 관찰가능성

시스템을 100% 관찰하고 싶지만, 샘플링·집계·전송 비용이 병목으로 작용합니다.
이에 따라 관찰가능성은 종종 통계적 추정 문제로 환원됩니다.

### 5.3.1 추정 오차의 하한 (Cramér–Rao Bound)

모수 θ를 추정할 때, 어떤 불편추정량 θ(hat)의 분산은 
피셔 정보 I(θ)로 아래와 같은 하한을 갖습니다:

$$
\mathrm{Var}(\hat{\theta}) \ge \frac{1}{I(\theta)}
$$

즉, 관측량(샘플 수)을 늘리지 않고는 특정 정밀도 이하로 내려갈 수 없다는 말입니다.

### 5.3.2 스케치/요약 자료구조의 근사성

* Count-Min Sketch: 주파수 추정 오차 

$\varepsilon$

를 얻으려면 폭 

$w \approx e/\varepsilon$
...
가 필요합니다. 
즉 메모리와 정확도 간 명시적 트레이드오프가 발생한다고 볼 수 있습니다.
* HyperLogLog: 고유 개수 추정의 표준오차 

$\approx 1.04/\sqrt{m}$ 

(레지스터 m). 
즉, 정확도를 두 배로 개선하려면 4배의 메모리가 필요하다는 것입니다.

이번에도 실제 사레를 살펴보면,
CloudWatch(AWS) / Cloud Monitoring(GCP)는 지표의 다운샘플링 작업을 선행합니다.
고해상도(예: 1s)로 오래 보존하려면 비용과 저장 한계가 기하급수적으로 증가하기 때문입니다!
또한 OpenTelemetry 같은 대규모 분산 트레이싱은 헤드/테일 샘플링으로 비용을 절감하는데
이는 곧 근사 추정을 전제로 합니다.

결론적으로 “모든 것을 본다”는 목표는 정보이론·비용 제약으로 불가능합니다.
즉, 얼마나 정확히, 얼마나 싸게 볼 것인가가 과제입니다!

---

## 5.4 운영의 수학: SLO·리스크·비용의 다목적 최적화

AE의 의사결정은 본질적으로 다목적 최적화(Multi-Objective Optimization)입니다.
...
현실에서는 이 문제를 정확히 풀 수 없습니다.
대신 Pareto Optimal 해들을 탐색해 정책으로 굳힙니다.


다시 AWS 실제 사례를 들여다보면 EC2 Auto Scaling과 Spot을
활용하면 비용을 대략 60~90% 절감 가능하지만 중단 리스크가 증가하게 됩니다.
(즉 언제 회수될 지 모르게 때문에 중단 실행 전 컨텍스트 저장이 필요하다는 것입니다!)
보통은 혼합 포트폴리오(온디맨드 $+$ 스팟 $+$ 리저브드)를 구성해 CVaR(조건부 가치-at-리스크)를 낮춥니다.
GKE Autopilot도 유사하게 클래스별 SLO/비용 제약을 고려한 정책을 택하고 있습니다.
...
만약 $P=NP$라고 가정해보면 어떨까요?
아마도 세상은 근본적으로 완전히 달라지게 될겁니다.

먼저 암호학의 붕괴입니다.
현 시점의 RSA, ECC 등 공개키 암호는 소인수분해, 이산로그 문제가 다항시간에 풀 수 없다는 가정에 의존하고 있습니다.
그렇기 때문에 $P=NP$라면, 이 가정이 순식간에 무너지고 인터넷 보안의 기반도 더이상 존립하기 어렵게 됩니다.

다음은 AI의 완전 자동화입니다.
프로그램 자동 생성, 수학 증명 자동화, 단백질 접힘, 신약 개발… 등
모든 NP 문제를 다항시간 내 정밀하게 풀 수 있게 되면서
사실상 “완전한 자동 프로그래머”의 출현이 가능하게 됩니다.

마지막은 자연과학 난제 해결입니다.
300년가 풀리지 않고 있는 Navier–Stokes 방정식의 해부터 
단백질 접힘, 물리·화학적 시뮬레이션의 정밀 계산이 현실화될 수 있습니다.
즉 인간이 수천 년간 싸운 문제들이 단숨에 tractable해지고 난제는 존재하지 않게 되는 것이죠.

다시 생각해보면 $P=NP$는 단순한 수학적 동치가 아니라, 
문명 전체의 규칙이 다시 쓰이는 사건이라고 보는 것이 맞습니다.

## 6.3 P≠NP라면?

기대와는 달리 대부분의 학자들은 $P \ne NP$에 무게를 두고 있습니다.
이는 곧 복잡도가 우리의 운명적 제약임을 시사하고 있죠.

물론 현재와 비슷하게 근사 알고리즘의 필연성도 인정하게 됩니다.
다항시간 근사 알고리즘(Approximation Algorithm)의 연구는 곧바로 $P \ne NP$ 전제할 수 있고
어떤 NP-hard 문제 $X$에 대해, “해의 질”과 “계산 시간” 사이의 근사비율 추구의 경우도
...
완벽한 해 대신 “얼마나 가까운가”가 핵심 척도로 작용할 것입니다.

또한 평균적 난이도와 휴리스틱은 기본이 됩니다.
실무에서 우리가 푸는 문제는 최악의 경우(worst-case)가 아니라 평균적 경우(average-case)로 귀결되고
랜덤화 알고리즘, 휴리스틱, 메타휴리스틱(유전 알고리즘, 시뮬레이티드 어닐링 등)이 주류 전략이 될 것입니다.

추가로 자연의 제약과 병행도 고려하게 됩니다.
자연 또한 “완전한 해”가 아니라 “충분히 좋은 근사해”를 선택한다는 잠정적 결론에 도달하기 때문에
진화 알고리즘의 경우 NP-hard한 공간에서 근사적 적합도를 찾는 과정으로 해석할 수 있습니다.

## 6.4 복잡도의 철학: 불가능을 운영하는 기술

따라서 P vs NP 문제는 단순한 이론적 질문이 아니라,
소프트웨어 엔지니어가 직면한 본질적 태도를 드러냅니다.
...
결국 “엔지니어링이란 풀 수 없는 문제를 근사적으로 운영하는 기술”입니다.

---

## 6.5 결론을 향해

만약 $P=NP$라고 가정한다면 우리의 문명은 그 즉시 새로운 질서로 전환하게 됩니다.
반대로 $P\ne NP$라면 우리는 불가능과 제약 속에서 근사, 휴리스틱, 타협, 운영을 지속해야한다는 말이되죠.

즉, 복잡도는 단순한 이론이 아니라,
자연과 코드가 공유하는 궁극적 법칙,
그리고 엔지니어의 운명적 제약이라고 보는것이 맞습니다.

# 7장. 결론 – 소프트웨어 엔지니어의 길

우리는 지금까지 복잡도의 그림자가 데이터, 프론트엔드, 백엔드, 아키텍처 모든 층위에 드리워져 있음을 살펴보았습니다.
결론은 단순합니다.

“소프트웨어 엔지니어링이란 풀 수 없는 문제를 근사적으로 운영하는 기술”이라고 말할 수 있습니다.
...
모두 “근사(approximation), 휴리스틱(heuristics), 트레이드오프(trade-offs)”를 통해 현실화됩니다.
...
엔지니어링은 이 축 위에서 Pareto 최적점을 탐색하는 과정입니다.

---

## 7.3 근사의 미학

P≠NP일 가능성이 높다는 사실은 좌절이 아니라, 오히려 창의성의 원천입니다.
풀 수 없는 문제를 다루는 과정에서 다음과 같은 중요한 요소들을 배울 수 있기 때문이죠.

1. 완전해 대신 “쓸모 있는 해”를 찾는 지혜
2. 휴리스틱 설계에서 오는 경험적 직관
3. 제약 조건 속에서 새로운 알고리즘과 패턴을 발명하는 창의성
...
따라서 소프트웨어 엔지니어의 역할은 단순한 코드 작성이 아닙니다.
우리는 자연의 법칙과 수학적 제약이 허락한 범위 안에서,
가장 좋은 근사해를 가장 우아하게 운영하는 기술자라고 보는 것이 맞습니다.

이전에 언급한 내용을 바탕으로 정리해보면

데이터 사이언티스트는 NP-hard 최적화 대신 SGD를 선택!
프론트엔드 엔지니어는 NP-complete한 DOM diffing 대신 Virtual DOM을 선택!
백엔드 엔지니어는 CAP의 불가능성 속에서 CP 또는 AP를 선택!
아키텍처 엔지니어는 Bin Packing과 정보이론적 한계 속에서 휴리스틱과 샘플링을 선택!

즉, 우리가 쓰는 모든 기술은 “근사”라는 공통 언어 위에 세워져 있다고 결론 내릴 수 있습니다.
