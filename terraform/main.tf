...
data "template_cloudinit_config" "gpu_userdata" {
  gzip          = false
  base64_encode = true
  part {
...
resource "aws_launch_template" "gpu_lt" {
  ...
  network_interfaces { subnet_id = var.subnet_id, security_groups=[var.security_group], associate_public_ip_address=true }
  block_device_mappings {
    device_name = "/dev/xvda"
    ebs { volume_size = 200, volume_type = "gp3", delete_on_termination = true }
  }
  user_data = data.template_cloudinit_config.gpu_userdata.rendered
}
...
resource "aws_autoscaling_group" "gpu_asg" {
  ...
  mixed_instances_policy {
    ...
    launch_template {
      ...
      overrides { instance_type = "g5.xlarge" }
    }
  }
}

# Orchestrator (on-demand)
data "template_cloudinit_config" "cpu_userdata" {
  gzip          = false
  base64_encode = true
  part { content_type = "text/x-shellscript", content = file("${path.module}/userdata_cpu.sh") }
}
resource "aws_instance" "orchestrator" {
  ...
  user_data              = data.template_cloudinit_config.cpu_userdata.rendered
