...
Purpose: Test if α learns away from 0.5 on synthetic point cloud data
Key Question: Does the model learn to prefer learned attention vs spatial bias?

Expected Behavior:
- α ≈ 0.5: Balanced mixing (like WikiText)
- α > 0.6: Learned attention dominates
- α < 0.4: Spatial bias dominates (our goal!)
"""
import sys
import os
...
class SimplePointClassifier(nn.Module):
    ...
    """
    def __init__(self, n_classes=20, d_model=64, n_heads=8, use_ascender=False):
        super().__init__()
        ...
        if use_ascender:
            cfg = PointAscenderConfig(
                ...
                per_head_scale=True,
                per_head_gate=True,
                w_align=0.40,   # Stronger than before
                w_sep=0.20,
                w_coh=0.30,
                sigma_sep=0.05,
                sigma_coh=0.50,
...
def train_and_analyze(n_epochs=50, batch_size=16, device='cpu'):
    """
    Train baseline vs ASCender and compare α learning
    """
    print("="*80)
    print("🚀 RBP (Residual Bias Path) Learning Test")
    print("="*80)
    print()
    ...
    if mean_alpha > 0.6:
        ...
        print("   → ASCender bias provides minimal value in this synthetic task")
        print()
        print("💡 Implications:")
        print("   - Synthetic data may be too simple")
        print("   - Need real spatial structure (MSRAction3D)")
        print("   - OR increase bias strength (w_align, w_sep, w_coh)")
        ...
        print("   → Similar to WikiText results")
        print()
        print("💡 Implications:")
        print("   - Architecture works correctly")
        print("   - But bias strength may be too weak")
        print("   - Try increasing w_align, w_sep, w_coh by 2x")
    ...
    if alpha_drift > 0.1:
        ...
        print("   → Model didn't find strong preference")
        print("   → May need longer training or stronger bias")
    ...
    if mean_alpha < 0.4:
        ...
        print("   1. Download MSRAction3D dataset")
        print("   2. Implement data loader")
        print("   3. Run full experiments")
    else:
        print("   ⚠️  Need adjustments:")
        print("   1. Increase bias strength (w_align=0.6, w_sep=0.3, w_coh=0.5)")
        print("   2. Re-run this test")
        print("   3. If still α ≈ 0.5, try real data (MSRAction3D)")
    print()
